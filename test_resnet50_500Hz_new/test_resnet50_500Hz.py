...
def parse_args():
    ...
    parser.add_argument('--epoch_for_save', type=int, default=60)  # 记得改epoch数
    ...
    parser.add_argument('--outputresult_dir', type=str, default="./0329_oldmodel_yqhdata_500hz/epoch60")  # 记得改输出路径，不用带/应该
...
def test_net(net, epoch, val_loader, writer, batch_size):
    ...
    with torch.no_grad():
        ...
        for j, datas in enumerate(progress_bar):
            ...
            #gt_t60_reshape = torch.load(names[0][0])[names_keys][0]['t60'][args.which_freq].tolist()
            #gt_t60_reshape = torch.tensor(np.array([gt_t60_reshape] * gt_t60_reshape_len))
            gt_t60_reshape = gt_t60_reshape.T[:, :, args.which_freq].T  # [total_slices, 1]
            ...
            for i in range(len(valid_len)):
                ...
                #mean_gt = torch.tensor(gt_t60_reshape).to(torch.float32).to(device).tolist()
                mean_gt_value = torch.mean(gt_t60_reshape.T[args.which_freq], dim=0).tolist()
                mean_gt = [0, 0, 0, 0, 0, 0]
                for ii in range(6):
                    mean_gt[ii] = mean_gt_value
                mean_gt = torch.tensor(mean_gt)
                mean_gt = torch.tensor([sum(mean_gt)/len(mean_gt)])
                mean_bias = torch.tensor([sum(mean_bias)/len(mean_bias)])
                mean_rsquare_error = torch.tensor([sum(mean_rsquare_error)/len(mean_rsquare_error)])
...
def get_boxplot(test_res):
    ...
    for key, value in test_res.items():
        ...
        temp = np.array(torch.load(key)[room][0]['t60'][args.which_freq].unsqueeze(0)).tolist()
        print(temp)
        print(torch.load(key)[room][0]['MeanT60'][args.which_freq])
