...
def parse_args():
    ...
    parser.add_argument('--model_path', type=str,
                        default="/mnt/sda/xbj/t60_resnet101/save_model/resnet_1000Hz/")  # 记得改成对应的路径，最后要带/
    ...
    parser.add_argument('--which_freq', type=int, default=3)
    ...
    parser.add_argument('--SERVER', type=bool, default=False)  # 在服务器上运行记得改成True
    parser.add_argument('--outputresult_dir', type=str, default="resnet101_test")  # 记得改输出路径，不用带/应该
...
def get_boxplot(test_res):
    ...
    ticks = room_dict.keys()
    gt_plot = plt.boxplot([v['gt'] for _, v in room_dict.items()],
                          positions=np.array(np.arange(len(room_dict.values()))) * 2.0, widths=0.3)
    ...
    plt.xticks(np.arange(0, len(ticks) * 2, 2), ticks, fontsize=20)
...
if __name__ == "__main__":
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    if not args.SERVER:
        model_path = "/Users/bajianxiang/Desktop/internship/new_dataset_t60_attention/model/resnet101-5d3b4d8f.pth"
    else:
    ...
    if DEBUG == False:
        ...
        failed_file = "/Users/bajianxiang/Desktop/internship/filter_down_spec_dataset/koli-national-park-winter/koli-national-park-winter_koli_snow_site4_1way_bformat_1_koli-national-park-winter_东北话男声_1_TIMIT_a005_100_110_20dB-0.pt"
    else:
