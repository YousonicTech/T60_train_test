...
if __name__ == "__main__":
    ...
    if not is_parallel:
        ...
        # 并行
        commands = ["CUDA_VISIBLE_DEVICES=\""+ str(i%3) + "\" " + "python test_FPN_STI.py --epoch_for_save " + str(epochs[i]) + " --outputresult_dir " + "./0925_test_lr2001/epoch" + str(epochs[i]) for i in range(len(epochs))]
        threads = []
        for cmd in commands:
