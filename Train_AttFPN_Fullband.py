...
import os
import glob
from new_data_load_original_FULLBAND import Dataset_dict, collate_fn
...
def parse_args():
    ...
    parser.add_argument('--save_dir', type=str, default='/data2/hsl/T60_train_test/save_model/0707_FULLBAND3_attFPN_old_hybl_zgc_yqh_zgcfx_NONoise_3040_1_2_random/')
    parser.add_argument('--model_path', type=str, default="/data2/hsl/T60_train_test/save_model/0704_FULLBAND3_attFPN_old_hybl_zgc_yqh_zgcfx_NONoise_3040_1_2_random")
    parser.add_argument('--load_pretrain', type=bool, default=False)
    # 125Hz:(0, 7); 250Hz:(1, 10); 500Hz:(2, 13); 1kHz:(3, 16); 2kHz:(4, 19)
    parser.add_argument('--which_freq', type=int, default=4)  # 换倍频程,用来选择语谱图，现在不需要了
    parser.add_argument('--freq_loc', type=int, default=[13,16, 19])  # [13,16,19]
    parser.add_argument('--ln_out', type=int, default=3)#训练几个频段就写几
...
def val_net(net, epoch, val_loader, writer):
    ...
    with torch.no_grad():
        ...
        for j, datas in enumerate(progress_bar):
            ...
            clean_reshape = datas['clean'].to(torch.float32).to(device).squeeze()
            gt_t60_reshape = gt_t60_reshape.T[args.freq_loc].T  # [total_slices, 1]
            #print(images_reshape.shape[0])
        ...
        print("Epoch %d Evaluation: mean mse loss:%f,mean bias:%f" % (epoch, mean_mse_loss, mean_bias))
...
def train_net(start_epoch, n_epochs, train_loader, val_loader, batch_size, args):
    ...
    temp = args.save_dir.split('/')[-2]
    log_dir = os.path.join("./log/"+temp+'/')
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        for batch_i, datas in enumerate(progress_bar):
            ...
            images_reshape = datas['image'].to(torch.float32).to(device)
            gt_t60_reshape = datas['t60'].to(torch.float32).to(device).unsqueeze(1)
            gt_t60_reshape = gt_t60_reshape.T[args.freq_loc].T.squeeze()
...
if __name__ == "__main__":
    ...
    if LOAD_PRETRAIN == True:
        ...
        net, trained_epoch = load_checkpoint(model_path, 74, net, device)
    ...
    if DEBUG == False:
        ...
        #train_dict_root4 = "/data3/hsl/PT/No_Noise/YQH_norm_bu_pt/"
        #train_dict_root3 = "/data3/hsl/PT/No_Noise/ZGC_norm_bu_pt/"
        #train_dict_root2 = "/data3/hsl/PT/No_Noise/ZGCFX_norm_bu_pt/"
        train_dict_root = "/data3/hsl/PT/No_Noise/2K/"
        train_dict_root1 = "/data3/hsl/PT/3040dB/2K/"
        train_dict_root2 = "/data3/hsl/PT/No_Noise/1K/"
        train_dict_root3 = "/data3/hsl/PT/3040dB/1K/"
        train_dict_root4 = "/data3/hsl/PT/No_Noise/500_zdm/"
        train_dict_root5 = "/data3/hsl/PT/3040dB/500_zdm/"

        # # train_dict_root1 = None
        # # train_dict_root2 = None
        # # train_dict_root3 = None
        # train_dict_root4 = None
        val_dict_root = "/data3/hsl/TEST_PT/1K/LiveRecord_QTQW/"
        val_dict_root1 = "/data3/hsl/TEST_PT/2K/LiveRecord_QTQW/"
        batch_size = 3
        val_batch_size = 10
        failed_file = "/data3/hsl/TEST_PT/LiveRecord_QTQW//1K/val_hybl_LiveRecord_QTQW/six-eight/six-eight-ch2-0_35.pt"

    print("train_dir0:", train_dict_root)
    print("train_dir1:", train_dict_root1)
    print("train_dir2:", train_dict_root2)
    print("train_dir3:", train_dict_root3)
    print("train_dir4:", train_dict_root4)
    print("train_dir5:", train_dict_root5)

    train_transformed_dataset = Dataset_dict(root_dir=train_dict_root, choose_data=True,choose_num=5000,
                                             root_dir1=train_dict_root1,choose_data1=True,choose_num1=10000,
                                             root_dir2=train_dict_root2,choose_data2=True,choose_num2=5000,
                                             root_dir3=train_dict_root3,choose_data3=True,choose_num3=10000,
                                             root_dir4=train_dict_root4,choose_data4=True,choose_num4=5000,
                                             root_dir5=train_dict_root5, choose_data5=True, choose_num5=10000,
                                             transform=data_transform,
                                             start_freq=args.start_freq, end_freq=args.end_freq,
                                             which_freq=args.which_freq, failed_file=failed_file,train=True)

    print("After All Preprocess,len of train dataset:", len(train_transformed_dataset))

    val_transformed_dataset = Dataset_dict(root_dir=val_dict_root,root_dir1=val_dict_root1, transform=data_transform,
                                           start_freq=args.start_freq,end_freq=args.end_freq, which_freq=args.which_freq,
