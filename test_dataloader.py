...
class Dataset_dict(Dataset):
    ...
    def __init__(self, root_dir=None, transform=None, start_freq=0, end_freq=29, which_freq=0, failed_file=".", random_choose_slice=True, channel3=True):
        """
        Args:
        ...
        for (root, dirs, files) in os.walk(root_dir):
            ...
            self.dict_data.extend(os.path.join(root, filen) for filen in files if filen.endswith('.pt'))
            self.root = root
        ...
        for list_c in range(len(dict_data)):
            if dict_data[list_c] == 0:
                ...
                if "image" in dict_data[list_c].keys():
                    ...
                    temp_image = temp_image[self.which_freq][temp_image[1].shape[0] // 2:].unsqueeze(0)
                    
                    images_list.append(temp_image)  # 选图的上半部分，shape=[1, 65, 175] for 2khz, 175是因为只有一个slice(4s)
                    
                    t60_list.append(torch.unsqueeze(dict_data[list_c]['t60'][self.freq_start:self.freq_end+1], 0))
                    ddr_list.append(torch.unsqueeze(dict_data[list_c]['ddr'][self.freq_start:self.freq_end+1], 0))
