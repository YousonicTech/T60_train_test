            ...
            if dict_data[list_c] == 0:
                ...
                if "image" in dict_data[list_c].keys():
                    if isinstance(dict_data[list_c]['image'],list):#如果是list，说明是新数据，因为有5个频段
                        ...
                        t60_list.append(torch.unsqueeze(dict_data[list_c]['t60'][self.freq_start:self.freq_end+1], 0)) #by hsl
                        ddr_list.append(torch.unsqueeze(dict_data[list_c]['ddr'][self.freq_start:self.freq_end+1], 0)) #by hsl
                        ...
                        t60_list.append(
                            torch.unsqueeze(dict_data[list_c]['t60'][self.freq_start:self.freq_end+1], 0))  # by hsl
                        ddr_list.append(
                            torch.unsqueeze(dict_data[list_c]['ddr'][self.freq_start:self.freq_end+1], 0))  # by hsl
                        # t60_list.append(torch.unsqueeze(dict_data[list_c]['t60'][13], 0))#对应500hz
                        # ddr_list.append(torch.unsqueeze(dict_data[list_c]['ddr'][13], 0))
                        MeanT60_list.append(torch.unsqueeze(dict_data[list_c]['MeanT60'], 0))
                        name_list.append(dict_data_name)
