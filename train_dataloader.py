...
class Dataset_dict(Dataset):
    ...
    def __init__(self, root_dir=None,clean_dir=None, transform=None, start_freq=0, end_freq=29, which_freq=0, failed_file=".", random_choose_slice=True, channel3=True):
        """
        Args:
    ...
    def __getitem__(self, idx):
        ...
        #print('-----------------------tttt6660000 ====',dict_data[0]['t60'])
        ...
        # PYQ data以 "_TIMIT" 为标志，将文件名分为两部分
        if "TIMIT" in dict_data_name:
            
            parts = dict_data_name.split("_TIMIT")
            # 提取往前第二个 "_" 之间的字符,就是clean的名字\
            clean_data_name = self.clean_dir + parts[0].split("_")[-2] + '-' + parts[0].split("_")[-1] + '.pt'
        ...
        # my data
            ...
            #parts = dict_data_name.split("_TIMIT")
            clean_name = dict_data_name.split("/")[-1].split("_")[-3]
                # 提取往前第二个 "_" 之间的字符,就是clean的名字\
                # print(parts)
            clean_data_name = self.clean_dir + clean_name + '.pt'
            clean_data = torch.load(clean_data_name)
            clean_keys = list(clean_data.keys())[0]
            clean_data = clean_data[clean_keys]
            ...
            if dict_data[list_c] == 0:
                ...
                if "image" in dict_data[list_c].keys():
                    if isinstance(dict_data[list_c]['image'],list):#如果是list，说明是新数据，因为有5个频段
                        ...
                        # temp_clean = torch.Tensor(temp_clean)
                        temp_clean = temp_clean[temp_clean.shape[0] // 2:].unsqueeze(0)
...
def collate_fn(batch):
    ...
    for k in stacking_keys:
        #print("key:",k)
        #print(out[k])
        # print("MeanT60.shape:",out['MeanT60'][0].shape)
        # print("t60.shape:",out['t60'][0].shape)
        # for i in range(len(out[k])):
        
            # if out['MeanT60'][i].shape[2] > 30:
            #     out['MeanT60'][i] = out['MeanT60'][i][:,:,:30,:]
            # if out['t60'][i].shape[1] > 30:
            #     out['t60'][i] = out['t60'][:,:30]
...
if __name__ == "__main__":
    train_dict_root = "/data1/zdm/T60_500Hz_Data_pt/train/PYQ_NoNoise/Echo/Brutalism"
    data_transform = transforms.Compose([transforms.Resize([224, 224])])
    failed_file = "/data1/zdm/T60_500Hz_Data_pt/train/3040Noise/Echo_3040noise/Miscellaneous/Miscellaneous_AcademicQuadrangle-left_东北话男声-1_a109-0-10_30dB-0.pt"
    clean_dir="/data1/zdm/CleanSpeech/speech_pt/CatChinese/"
    ...
    print(train_transformed_dataset[0]["image"].shape)
