...
import os
import glob
from test_dataloader import Dataset_dict, collate_fn
...
#TODO: 修改model_path,test_epochs,test_freq,outputresult_dir_head(这个可以不用动）
def parse_args():
    parser = argparse.ArgumentParser(description='Evaluate the mmTransformer')
    parser.add_argument('--model_path', type=str,
                        default="/data1/zdm/code/WithEnhancement/NoNoiseAndOne3040Model/",#
                        help= '保存的模型路径')
    parser.add_argument('--test_epochs', type=list, default=[37],help='要测试的epoch，列表')  # 记得改epoch数
    parser.add_argument('--test_freq', type=int, default=500,choices=[500,1000,2000,4000])
    ...
    parser.add_argument('--outputresult_dir_head', type=str,default="/data1/zdm/code/WithEnhancement/LiveRecord/",help='输出测试结果的路径')
    args = parser.parse_args()
    return args
#TODO:修改val_dict_root_list,failed_file_list

val_dict_root_list=["/data1/zdm/T60_500Hz_Data_pt/val/LiveRecord/val_YQH_LiveRecord/",
                    "/data1/zdm/T60_500Hz_Data_pt/val/LiveRecord/val_hybl_LiveRecord/",
                    "/data1/zdm/T60_500Hz_Data_pt/val/val_ZGCFX_LiveRecord/"
                    ]

failed_file_list =["/data3/hsl/TEST_PT/LiveRecord_QTQW/val_ZGCFX_LiveRecord_QTQW/ZGCFX-1-3/ZGCFX-1-3-ch4-0_44.pt",
                    "/data3/hsl/TEST_PT/LiveRecord_QTQW/val_hybl_LiveRecord_QTQW/eight-two/eight-two-ch2-0_0.pt",
                   "/data3/hsl/TEST_PT/LiveRecord_QTQW/val_YQH_LiveRecord_QTQW/YQH101/YQH101-ch1-0_0.pt",
                                      ]


def get_freq_index(test_freq):
...
def test_net(net, epoch, val_loader, writer, batch_size):
    ...
    with torch.no_grad():
        ...
        print("epoch:", epoch, "RMSE:", math.sqrt(mean_loss))
        # print("Epoch %d Evaluation: mean loss:%f,mean bias:%f" %(epoch,mean_loss,mean_bias))
...
if __name__ == "__main__":
    ...
    for test_epoch in args.test_epochs:
        ...
        for i in range(len(val_dict_root_list)):
            ...
            choose_data = False
            print(val_dict_root)
            val_transformed_dataset = Dataset_dict(root_dir=val_dict_root, transform=data_transform,
                                                   start_freq=args.start_freq,
                                                   end_freq=args.end_freq, which_freq=which_freq,
                                                   failed_file=failed_file,
                                                   random_choose_slice=False)#_20dB
