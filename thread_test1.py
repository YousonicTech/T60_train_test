...
if __name__ == "__main__":
    ...
    if not is_parallel:
        ...
        # 并行
        commands = ["CUDA_VISIBLE_DEVICES=\""+ str(i%3) + "\" " + "python test_resnet50_500Hz.py --epoch_for_save " + str(epochs[i]) + " --outputresult_dir " + "./1011_test/epoch" + str(epochs[i]) for i in range(len(epochs))]
        threads = []
        for cmd in commands:
